...
apk 内置频道列表，以及生成它的那次测试留下的全部原始记录。
...
- `current.m3u` — 上面这份列表的标准 m3u 形式，方便丢进 VLC、PotPlayer 之类的播放器里看。
  由 `tools/export_m3u.py` 生成，内容和 apk 实际播的完全一致（包括凤凰的本地转发地址）。
...
## 重新生成
    ...
    python3 tools/extract_test.py --serial <tv-ip>:5555   # 重测，刷新本目录三个文件
    python3 tools/merge.py                                # 写回 app/.../raw/channels.txt
    python3 tools/export_m3u.py                           # 刷新 current.m3u
    ./gradlew assembleRelease                             # 重新打包
