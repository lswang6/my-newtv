...
- 测试位置：这台 Mac，协议与 `tools/probe/extract_test.py --mac` 相同，**不是电视**。出口是路由器代理，落在韩国的 Oracle Cloud 节点，只有 IPv4；结果说明的是这个出口能连上什么，不代表国内家庭宽带或港台线路。
...
对照 README「直播源来源」表，也就是 `tools/probe/extract_test.py` 实际下载的 11 个源。
