...
- 设置里的「默认列表选择」：「恢复默认（119）频道」恢复全部设置和内置的 119 个频道；「切换到全球精简版」只把频道列表换成打包在 apk 里的 349 个频道的精简版（不联网，重启后保留），其他设置不动，每个列表的收藏分开保存，切换后再切回来仍在
...
（已于 2026-09-17 在电视上验证：旧收藏迁移、默认和精简版各自的收藏、来回切换、重启保留、有收藏时数字键选台。）

- 收藏按列表分开保存（切换、重启都保留，旧收藏自动迁移到当前列表）。「恢复默认（119）频道」和「切换到全球精简版」不再清空收藏。
- 切换列表后「收藏」分组立即刷新，之前要重启才换掉旧列表的收藏。
- 修复：有收藏时数字键选错台（频道号从「收藏」开始数，每个收藏都让号码错一位）。

已知问题（未修）：和 v2.2.0 里「显示「全部」时重新导入」同类，菜单停在「收藏」时换列表，收藏列表也会先变、后通知菜单。
...
- 移除频道：纬来体育（v2.1.2）、足球频道、精品体育。它们的 epg.pw 地址在源站离线时会播放一段带二维码广告的"无信号"垫片。
